...
123P implements a deterministic local proactive execution skeleton for governed proactive-origin delegations that were registered through the existing 111P-style authority path. It revalidates 122P adapter lineage, preserves 118P through 122P plus 100P/101P/102P and 107P through 111P lineage where applicable, creates only local inspectable execution attempts and local completion or failure event candidates, and keeps all output local and inspectable. 124P later added deterministic read-only daily brief snapshots and renderable local text only. 125P later added deterministic read-only skill pack activation surfaces only. 123P itself does not authorize 103P inbox insertion, 104P result surfaces, 105P Telegram delivery, acknowledgements, live connector reads, model/tool calls, worker dispatch, Memory Center mutation, or external effects. 126P and later remain unauthorized.
...
124P implements a deterministic local daily brief layer for existing Hermes local state only. It scopes records to one owner and one robot, filters to the requested time window, classifies the resulting local items into read-only brief sections, preserves safe lineage references, renders inspectable local text, and keeps all output local and inspectable. 125P later added deterministic read-only skill pack activation surfaces only. 124P itself does not authorize Telegram delivery, callback binding, follow-up intent creation, async delegations, execution, Memory Center mutation, memory proposal creation, model/tool calls, live connector reads, worker dispatch, or external effects. 126P and later remain unauthorized.
...
125P implements a deterministic local skill pack activation layer for existing Hermes local state only. It scopes records to one owner and one robot, classifies each eligible item into a deterministic skill pack category, preserves safe lineage references, renders inspectable local pack counts and item mappings, and keeps all output local and inspectable. It does not authorize billing, entitlement enforcement, package activation, upgrade prompts, Telegram delivery, callback binding, follow-up intent creation, async delegations, execution, Memory Center mutation, model/tool calls, live connector reads, worker dispatch, or external effects. 126P and later remain unauthorized.
