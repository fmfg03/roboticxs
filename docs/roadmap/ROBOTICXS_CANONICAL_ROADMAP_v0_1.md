...
```json canonical-roadmap-authority
{
  ...
  "local_evidence_scope":"stages_61P_through_140P",
...
133P is a local manual connector increment only. It authorizes a read-only Google Calendar fetch and deterministic local CLI rendering. 134P later authorizes bounded read-only Calendar backing for owner-gated Telegram `/brief` replies only. It does not authorize Calendar writes, model/tool calls, worker dispatch, Memory Center mutation, async delegation, billing, entitlement enforcement, or non-Telegram external writes. 135P later added a reusable local real Calendar meeting brief composer/CLI only. 139P later added owner-requested suggested meeting brief rendering only. 140P later added a DeerFlow docs/test-only pattern review only. 141P and later remain unauthorized.
...
134P is a bounded owner-facing Telegram command surface increment only. It authorizes `/brief` to include read-only Google Calendar snapshot lines from the existing 133P connector and to fail closed to local deterministic meeting context when Calendar is unavailable. It does not authorize Calendar create/update/delete, Memory Center mutation, model/tool calls, worker dispatch, async delegation, billing, entitlement enforcement, or non-Telegram external writes. 135P later added a reusable local real Calendar meeting brief composer/CLI only. 139P later added owner-requested suggested meeting brief rendering only. 140P later added a DeerFlow docs/test-only pattern review only. 141P and later remain unauthorized.
...
135P is a local composer increment only. It authorizes deterministic local meeting brief records and CLI rendering from read-only Google Calendar snapshots. It does not authorize Telegram command changes beyond the existing 134P behavior, Calendar create/update/delete, Memory Center mutation, model/tool calls, worker dispatch, async delegation, billing, entitlement enforcement, or external writes. 139P later added owner-requested suggested meeting brief rendering only. 140P later added a DeerFlow docs/test-only pattern review only. 141P and later remain unauthorized.
...
137P is a local read-only scan increment only. It may detect review-only context candidates such as business context, meeting-prep context, or relationship context from Calendar event metadata. It does not write ProposedMemory or Memory Center records automatically. It does not change Telegram behavior, create approvals, trigger proactive suggestions, create follow-up intents, call models/tools, dispatch workers, write Calendar data, bill, enforce entitlements, or write external systems. 139P later added owner-requested suggested meeting brief rendering only. 140P later added a DeerFlow docs/test-only pattern review only. 141P and later remain unauthorized.
...
139P is a bounded owner-facing Telegram command surface increment only. It does not change bare `/brief` behavior, does not execute `/brief` automatically from `/suggest_brief`, and does not bind callbacks, create follow-up intents, delegate or dispatch workers, mutate Memory Center or ProposedMemory, call models or tools, write Calendar data, bill, enforce entitlements, or write external systems beyond approved Telegram replies. 140P later added a DeerFlow docs/test-only pattern review only. 141P and later remain unauthorized.
...
140P is a docs/test-only roadmap and architecture review increment. It does not install, clone, import, execute, or integrate DeerFlow. It does not change Telegram behavior, Memory Center behavior, model routing, tool authority, worker dispatch, connectors, sandbox execution, billing, entitlement enforcement, or external network behavior. 141P and later remain unauthorized.
