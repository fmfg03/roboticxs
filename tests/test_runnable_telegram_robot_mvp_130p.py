...
def test_130p_startup_report_is_deterministic():
    ...
    assert "Pilot Review Session Pack: /pilot_review summarizes local pilot session learnings only" in report
    assert "Pilot Learning Queue: /pilot_learnings shows prioritized local product learnings only" in report
    assert "Paid Pilot Readiness Gate: /paid_pilot_gate evaluates local paid-pilot readiness without billing" in report
