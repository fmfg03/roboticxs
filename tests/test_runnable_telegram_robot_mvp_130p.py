...
def test_130p_startup_report_is_deterministic():
    ...
    assert "Available commands: /start, /help, /menu, /status, /checkup, /setup, /miss, /today, /daily_brief, /demo, /pilot, /pilot_pack, /pilot_audit, /live_smoke, /pilot_metrics, /friendly_onboarding, /friendly_pilot, /pilot_users, /pilot_user, /pilot_health, /pilot_invite, /pilot_consent, /pilot_provision, /pilot_allowlist, /pilot_boundary, /pilot_runbook, /report_issue, /report_bug, /report_confusing, /report_wrong, /report_missing, /report_slow, /founder_loop, /feedback, /feedback_ledger, /founder_outcome, /suggestion_quality, /prep_quality, /gmail_thread, /loops, /inbox, /inbox_done, /inbox_dismiss, /prep, /brief, /suggest_brief, /suggestions, /suggestion_dismiss, /suggestion_snooze, /suggestion_memory, /suggestion_draft, /suggestion_followup, /approvals, /approve, /reject, /drafts, /draft_approve, /draft_reject, /draft_edit, /draft_revise, /draft_expire, /export_text, /export_email, /export_file, /usage, /memory_review, /memory_approve, /memory_reject, /memory_edit, /memory_forget, /memory_wrong, /memory_stale, /memory_duplicate, /memory_merge, /memory_never_use, /memory, /memory_limits, /memory_pending, document upload" in report
    ...
    assert "Pilot Metrics Snapshot: /pilot_metrics shows local pilot metrics only" in report
    assert "Friendly User Onboarding Pack: /friendly_onboarding shows 1-3 user pilot setup only" in report
    assert "Founder-to-Friendly Pilot Baseline: /friendly_pilot shows the controlled pilot baseline only" in report
    assert "Friendly Pilot Operator Console: /pilot_users, /pilot_user, and /pilot_health show local pilot status only" in report
    assert "Friendly Pilot Invite & Consent: /pilot_invite and /pilot_consent show local consent text without sending invites" in report
    assert "Pilot User Provisioning: /pilot_provision and /pilot_allowlist show strict local allowlist receipts only" in report
    assert "Pilot Data Boundary: /pilot_boundary shows local owner/robot scope checks only" in report
    assert "Pilot Onboarding Runbook: /pilot_runbook shows the local Day 0-Day 7 pilot execution guide only" in report
    assert "Pilot Support Issue Capture: /report_* creates local pilot issue receipts only" in report
    ...
    assert "Daily Loop Outcome Tracker: /founder_outcome creates local outcome receipts only" in report
    assert "Suggestion Quality Tuning: /suggestion_quality shows local feedback-based ranking decisions only" in report
    assert "Prep Quality Tuning: /prep_quality shows local prep feedback tuning decisions only" in report
    assert "Draft Revision Loop: /draft_revise creates local revision candidates only" in report
