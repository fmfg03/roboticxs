...
def test_130p_status_from_authorized_owner_produces_deterministic_runtime_status():
    ...
    assert "Roadmap: 95P-163P closed, Gmail Read-Only Context Scan active" in receipt.reply_text
...
def test_130p_unknown_command_from_authorized_owner_produces_safe_fallback():
    ...
    assert "Setup Check: /status" in receipt.reply_text
...
def test_130p_startup_report_is_deterministic():
    ...
    assert "Available commands: /start, /help, /status, /miss, /today, /loops, /inbox, /inbox_done, /inbox_dismiss, /prep, /brief, /suggest_brief, /memory_approve, /memory_reject, /memory, /memory_limits, /memory_pending, document upload" in report
...
def test_130p_roadmap_registers_stage_and_133p_plus_block():
    ...
    assert "164P and later remain unauthorized" in roadmap
