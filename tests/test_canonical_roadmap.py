...
def test_stage_registry_contains_ordered_61p_through_66p2_and_79p_once():
    stages = load_stage_registry()
    stage_ids = [stage["stage_id"] for stage in stages]

    assert stage_ids == [f"{number}P" for number in range(61, 67)] + ["66P2"] + [
        f"{number}P" for number in range(67, 80)
    ]
...
def test_no_future_sequence_entries_claim_local_evidence_or_authorization():
    ...
    assert "80P" not in stages_by_id
...
def test_79p_transition_does_not_invent_80p():
    stages = load_stage_registry()
    ...
    assert [stage for stage in stages if stage["status"] == "NEXT_ELIGIBLE"] == []
    assert "80P" not in {stage["stage_id"] for stage in stages}
