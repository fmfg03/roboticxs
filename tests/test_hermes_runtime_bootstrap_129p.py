...
def test_129p_builds_runtime_online_status_for_valid_local_config():
    ...
    assert status.roadmap_closed_through == "205P"
...
def test_129p_text_report_contains_required_runtime_lines():
    ...
    assert "Roadmap: 95P-205P CLOSED_COMMITTED" in report.rendered_text
    ...
    assert "Next authorized stage: 206P+ remains unauthorized." in report.rendered_text
...
def test_129p_json_report_is_renderable():
    ...
    assert '"roadmap_closed_through": "205P"' in report.rendered_text
...
def test_129p_roadmap_registers_runtime_bootstrap_stage_and_135p_closed_with_136p_plus_block():
    ...
    assert "206P and later remain unauthorized" in roadmap
