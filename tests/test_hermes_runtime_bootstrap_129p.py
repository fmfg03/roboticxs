...
def test_129p_builds_runtime_online_status_for_valid_local_config():
    ...
    assert status.roadmap_closed_through == "226P"
    ...
    assert "customer_mvp_demo_pack_v1" in status.available_local_features
    assert "live_connector_readiness_check" in status.available_local_features
    assert "calendar_context_binding_v1" in status.available_local_features
    assert "gmail_context_binding_v1" in status.available_local_features
    ...
    assert "skill_manifest_runtime_gates" in status.available_local_features
    assert "controlled_live_pilot_baseline" in status.available_local_features
    assert "premium_telegram_ux_shell" in status.available_local_features
    ...
    assert "document_review_pack_v1" in status.available_local_features
    ...
    assert "user_confirmation_runtime" in status.available_local_features
    assert "approved_output_export" in status.available_local_features
    ...
    assert "gmail_thread_drilldown" in status.available_local_features
...
def test_129p_text_report_contains_required_runtime_lines():
    ...
    assert "Roadmap: 95P-226P CLOSED_COMMITTED" in report.rendered_text
    ...
    assert "Customer MVP Demo Pack v1: available for local end-to-end customer demo only" in report.rendered_text
    assert "Live Connector Readiness Check: available for owner-requested read-only readiness only" in report.rendered_text
    assert "Calendar Context Binding v1: available for owner-requested Calendar source trace only" in report.rendered_text
    assert "Gmail Context Binding v1: available for owner-requested Gmail source trace only" in report.rendered_text
    ...
    assert "Skill Manifest Runtime Gates: available for local command skill boundaries only" in report.rendered_text
    assert "Controlled Live Pilot Baseline: available for owner-requested controlled pilot receipts only" in report.rendered_text
    assert "Premium Telegram UX Shell: available for grouped customer-facing Telegram command surfaces only" in report.rendered_text
    ...
    assert "Document Review Pack v1: available for local injected-text document review only" in report.rendered_text
    ...
    assert "User Confirmation Runtime: available for owner-requested local confirmation receipts only" in report.rendered_text
    assert "Approved Output Export: available for owner-requested local export payloads only" in report.rendered_text
    ...
    assert "Gmail Thread Drilldown: available for owner-requested read-only thread metadata only" in report.rendered_text
    ...
    assert "Founder Feedback Capture: available for local non-persistent feedback receipts only" in report.rendered_text
    assert "Feedback Ledger & Tags: available for local structured feedback retrieval only" in report.rendered_text
    ...
    assert "Memory Correction Loop: available for local memory correction receipts only" in report.rendered_text
    assert "Pilot Metrics Snapshot: available for local pilot metrics only" in report.rendered_text
    assert "Friendly User Onboarding Pack: available for local 1-3 user onboarding only" in report.rendered_text
    assert "Founder-to-Friendly Pilot Baseline: available for local controlled pilot baseline only" in report.rendered_text
    assert "Friendly Pilot Operator Console: available for local Telegram/admin pilot visibility only" in report.rendered_text
    assert "Friendly Pilot Invite & Consent: available for local consent text only" in report.rendered_text
    assert "Pilot User Provisioning: available for strict local allowlist receipts only" in report.rendered_text
    assert "Pilot Data Boundary: available for local owner/robot scope checks only" in report.rendered_text
    assert "Pilot Onboarding Runbook: available for local Day 0-Day 7 execution guidance only" in report.rendered_text
    assert "Pilot Support Issue Capture: available for local pilot issue receipts only" in report.rendered_text
    assert "Pilot Safety Incident Log: available for local safety incident visibility only" in report.rendered_text
    assert "Pilot Weekly Report: available for local weekly pilot learning reports only" in report.rendered_text
    ...
    assert "Pilot Learning Queue: available for local prioritized pilot learnings only" in report.rendered_text
    assert "Paid Pilot Readiness Gate: available for local paid-pilot readiness decisions only" in report.rendered_text
    assert "Next authorized stage: 227P+ remains unauthorized." in report.rendered_text
...
def test_129p_json_report_is_renderable():
    ...
    assert '"roadmap_closed_through": "226P"' in report.rendered_text
